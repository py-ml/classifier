...
from clsfr.config import check_encrypted_text
from clsfr.config import get_boolean
from clsfr.config import get_config_data
from clsfr.config import get_integer, get_uint
from clsfr.config import settings
...
class ConfigTester(unittest.TestCase):
    ...
    @patch('clsfr.config.boto3')
    def test_check_encrypted_text(self, mock_boto3):
        """
        test clsfr.config.check_encrypted_text
        """
    ...
    @patch('clsfr.config.settings')
    def test_get_boolean(self, mock_settings):
        """
        test clsfr.config.get_boolean
        """
        tests = [
    ...
    def test_get_config_data(self):
        """
        test clsfr.config.get_config_data
        """
    ...
    def test_settings(self):
        ...
        config_dir = os.path.join(upper_path, "clsfr", "config.yaml")
        os.environ["DB_PORT"] = "13306"

        # reset Config singleton in order to mock with test data
        from clsfr.config import Config
        Config.reset()
